...
def calibrate_camera(images, boardShape=(8,6)):
    ...
    if len(objpoints) != len(imgpoints) or len(objpoints) == 0:
        raise ValueError("Number of object points and image points do not match or no valid corners found")
...
def straighten_image(image, K, dist_coeffs):
    ...
    map_x, map_y = cv2.initUndistortRectifyMap(K, dist_coeffs, None, new_K, (width, height), cv2.CV_32FC1)
