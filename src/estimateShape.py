...
def rectify_images_cylindrical(images, K):
    ...
    for i in range(len(images)):
        ...
        X = np.stack([u,v,np.ones_like(u)],axis=-1).reshape(height*width,3) # to homog
        Kinv = np.linalg.inv(K) 
        X = Kinv.dot(X.T).T # normalized coords

        # Calculate cylindrical coords (sin\theta, h, cos\theta):
        Xc = np.stack([np.sin(X[:,0]),X[:,1],np.cos(X[:,0])],axis=-1).reshape(width * height,3)
        ...
        rectified_image =  cv2.remap(img_rgba, B[:,:,0].astype(np.float32), 
